...
class PositionalEncoding(nn.Module):
    ...
    def __init__(self, embed_dim, dropout=0.1, max_len=5000):
        ...
        position = torch.arange(0, max_len).unsqueeze(1) # 完成向量的升维操作
        # 计算函数sin与cos内的内容, 参见论文的公式
        exp_term = torch.exp(torch.arange(0, embed_dim, 2) * -(math.log(10000) / embed_dim))
        # pe的计算, 相当于一个(N, T, P)的内容
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(self, query, key, value, attn_mask=None):
        ...
        # mask ? 
        if attn_mask is not None: # ~ 取反操作
          y[:, :, ~attn_mask.bool()] = float('-inf')

        y = self.softmax(y)
        y = self.dropout(y)

        # (N, H, S, d) -> (N, S, H, d) -> (N, S, D)
        y = torch.matmul(y, V).transpose(1,2).reshape(N, S, D) 
        output = self.proj(y)
