...
def get_optimizer(model):
    ...
    # *****END OF sYOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
    return optimizer
...
def ls_discriminator_loss(scores_real, scores_fake):
    ...
    real_label = torch.ones_like(scores_real).type(dtype)
    loss = 0.5*(
      torch.mean((scores_real-real_label)**2) + torch.mean(scores_fake**2)
    )
...
def ls_generator_loss(scores_fake):
    ...
    real_label = torch.ones_like(scores_fake).type(dtype)
    loss = 0.5 * torch.mean((scores_fake-real_label)**2)
