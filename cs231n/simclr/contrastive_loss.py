...
def sim(z_i, z_j):
    ...
    norm_dot_product = torch.matmul(z_i, z_j) / (
...
def simclr_loss_naive(out_left, out_right, tau):
    ...
    for k in range(N):  # loop through each positive pair (k, k+N)
        ...
        """loss_left = - torch.log(torch.exp(sim(z_k, z_k_N)/tau)/sum_k)
        loss_right = - torch.log(torch.exp(sim(z_k_N, z_k)/tau)/sum_k_n)
        total_loss += loss_left + loss_right"""

        s_kn = sim(z_k, z_k_N)

        loss_k = - torch.log(torch.exp(s_kn/tau) / sum_k)
        loss_k_n = - torch.log(torch.exp(s_kn/tau) / sum_k_n)

        total_loss = total_loss+loss_k+loss_k_n
...
def sim_positive_pairs(out_left, out_right):
    ...
    # NOTE: keepdim = True
    pos_pairs = torch.sum(
      out_left*out_right, dim=1, keepdim=True
    ) / (
...
def compute_sim_matrix(out):
    ...
    # similar with sim positive pairs
...
def simclr_loss_vectorized(out_left, out_right, tau, device='cuda'):
    ...
    denom = torch.sum(exponential, dim=1, keepdim=True).to(device)
    ...
    numerator = sim_positive_pairs(out_left, out_right).to(device)
    # *****START OF YOUR CODE (DO NOT DELETE/MODIFY THIS LINE)*****
