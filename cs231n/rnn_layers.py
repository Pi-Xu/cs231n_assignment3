...
def rnn_backward(dh, cache):
    ...
    for i in range(T-1, -1, -1):
      ...
      # NOTE: 此处需要将所有的加起来, 实际上所有的H可以看做是一个output, 然后在backward的过程中再sum所有的grads
      dx_i, dh0, dWx_i, dWh_i, db_i = rnn_step_backward(dh[:,i,:]+dh0, (x[:,i,:], prev_h, Wx, Wh, b))
      dx[:,i,:] = dx_i
      dWx = dWx_i + dWx
      dWh = dWh_i + dWh
      db = db_i + db
